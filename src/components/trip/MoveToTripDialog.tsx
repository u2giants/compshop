...
export default function MoveToTripDialog({ open, onOpenChange, photoIds, currentTripId, onMoved }: MoveToTripDialogProps) {
  ...
  const filtered = search
    ? trips.filter((t) => t.store.toLowerCase().includes(search.toLowerCase()) || t.name.toLowerCase().includes(search.toLowerCase()))
    : trips;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="font-sans">Move to Trip</DialogTitle>
          <DialogDescription>
        ...
        {loading ? (
          ...
          <p className="py-6 text-center text-sm text-muted-foreground">No other trips found.</p>
        ) : (
